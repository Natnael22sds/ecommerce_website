...
if (isset($_POST) && !empty($_POST)) {
    // Get email and hash the password using md5
    $email = $_POST['email'];
    $password = md5($_POST['password']);  // Hash the password using md5 to match the stored hash
    $usertype = 'Yana';

    // Prepare the SQL statement
    $sql = "SELECT * FROM admin WHERE email = ? AND password = ? AND usertype = ?";
