...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['agree']) && $_POST['agree'] === 'true') {
        ...
        if ($stmt->affected_rows > 0) {
            echo "Usermeta updated/inserted successfully.<br>";
            $total = 0;
            $productNames = [];
            ...
            $stmt = $connection->prepare("INSERT INTO orders (uid, totalprice, orderstatus, paymentmode, product_name, mobile, usertype, address1, country) VALUES (?, ?, 'Order Placed', ?, ?, ?, 'car', ?, ?)");
            if (!$stmt) {
                die('Prepare failed: ' . htmlspecialchars($connection->error));
            }
            ...
            if ($stmt->affected_rows > 0) {
                ...
                $account_sid = 'AC40aab2a7854baf068f5e35a622335a85';
                $auth_token = '002dbcba790dd1d62dda0eabc7cfa652'; 
                $twilio_number = "+14242173957"; 

                $client = new Client($account_sid, $auth_token);

             
                $adminPhone = '+251985586071';
            ...
            echo "Error: Could not update usermeta. SQL Error: " . htmlspecialchars($stmt->error);
        }
    }
}
